...
function _deleteAuthorizationHeader({error}) {
  if(error.response) {
    error.response = new global.Response(error.response, {
      headers: _replaceAuthorizationHeader({response: error.response})
    });
  }
  if(error.request) {
    error.request = new global.Request(error.request, {
      headers: _replaceAuthorizationHeader({request: error.request})
    });
  }
  return error;
}

function _replaceAuthorizationHeader({response, request}) {
  if(!(response || request)) {
    return new global.Headers();
  }
  const {headers: responseHeaders} = (response || request);
...
export async function makeHttpsRequest({
  ...
  if(result) {
    result = new global.Response(result, {
      headers: _replaceAuthorizationHeader({response: result}),
