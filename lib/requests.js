...
function _deleteAuthorizationHeader({error}) {
  if(error.response) {
    const newHeaders = _replaceAuthorizationHeader({response: error.response});
  ...
  if(error.request) {
    const newHeaders = _replaceAuthorizationHeader({request: error.request});
    const newRequest = new global.Request(error.request, {
      headers: newHeaders
    });
    error.request = newRequest;
  }
  return error;
}
